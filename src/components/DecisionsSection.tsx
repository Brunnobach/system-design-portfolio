...
export function DecisionsSection() {
  return (
    <section
      id="decisions"
      className="scroll-mt-24 border-t border-[var(--line)] bg-[var(--bg-elevated)] py-24 sm:py-32"
    >
      <div className="mx-auto max-w-6xl px-5 sm:px-8">
        ...
        <div className="mt-14 grid gap-px bg-[var(--line)] md:grid-cols-3">
          {adrs.map((adr, index) => (
            <Reveal key={adr.slug} delay={index * 90} className="h-full">
              <Link
                href={`/decisions/${adr.slug}`}
                className="group flex h-full flex-col bg-[var(--bg)] p-6 transition-colors hover:bg-[var(--bg-soft)]"
              >
